...
appConfig.type=1;//android or ios
appConfig.brandId = 1;
...
//		Toast.showShort("data:::"+JSON.stringify(data));
		break
		...
		if(versionNum>appConfig.versionCode){
	           ...
	           if(isFource==0){//非强制更新
	        	   navigator.notification.confirm(
                          ...
                          function(button){
	                          if(button==1){//
	                        	  window.open(data.versionOut.url,'_blank', 'location=yes');
...
//		log("user.sid==="+user.sid+";"+user.email+";"+user.password+";user.facebookId="+user.facebookId);
...
//		log("user.merchantId========"+user.merchantId);
		resetMainMenu(user);
//		goBack();
...
//		$("#store_detail_page #storeProvince").text(data.storeOut.city+" , "+data.storeOut.provinceShrtNm+" "+data.storeOut.zip);
...
//		log(JSON.stringify(data));
	...
	case COMMAND_DELETE_COLLECT_FROM_FAVORITE_PAGE:
		needHomePageRefresh=true;
//		var favoriteNum=parseInt($("#homepage #favoriteNum").text())-1;
//		$("#homepage #favoriteNum").text(orderNum);
		doCommandMyFavorites(appConfig,user);
//		$("#my_favorites_page #my_favorites_list").listview("refresh");
		break;
...
//		log(JSON.stringify(data));
		...
		$.each(data.listStroeCusCollect,function(i,item){
			item.opening=isStoreOpen(item);
//			var img=item.opening==0?"img/open.png":"img/closed.png";
			var openStatus="<span style='font-size:12px;color:red;font-weight:normal;margin-left:10px;'>[OPEN]</span>";
			if(item.opening!=0){
				openStatus="<span style='font-size:12px;color:#666;font-weight:normal;margin-left:10px;'>"+getStoreOpenRemainingTime(item)+"</span>";
			}
...
//			if(item.showOrder==categoryIndex){
			if(i==0){
...
//			var modName=item.addAmt==0?item.modName:(item.modName+" $"+item.addAmt.toFixed(2));
...
//					$("#dish_modifier_page #dish_modifier").append("<li class='modifiers' style='border-bottom-width: 1px;'><span style='white-space:normal;word-wrap:break-word;'>"+item.modName+"</span><span style='float:right;'><label>"+modAmt+"<input style='float:right;margin-top:-3px;margin-left:16px;' type='radio' "+checked+" value='"+itemStr+"' onchange='onRadioChange(this)' name='"+item.modCategory+"' /></label><span></li>");
...
//	storage.set('deviceId', user.deviceId);
	...
	user.birthDate=data.customerOut.birthDate;
	user.imgOriginalUrl=IMG_BASE_URL+data.customerOut.imgOriginalUrl;
//	log("user.imgOriginalUrl===="+user.imgOriginalUrl);
...
//		log("accessToken="+accessToken.accessToken+";userId="+accessToken.userId);
