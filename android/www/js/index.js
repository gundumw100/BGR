...
function toDirectionPage() {
	...
	if(curPosition==null){
		log("http://maps.google.com/maps?q=" + store.latitude + "," +store.longitude);
		window.open("http://maps.google.com/maps?q=" + store.latitude + "," +store.longitude, '_blank' );
	}else{
		log("http://maps.google.com/maps?f=d&source=s_d&saddr="+curPosition.coords.latitude+","+curPosition.coords.longitude+"&daddr="+store.latitude+","+store.longitude+"&hl=us&t=m&dirflg=d");
		window.open("http://maps.google.com/maps?f=d&source=s_d&saddr="+curPosition.coords.latitude+","+curPosition.coords.longitude+"&daddr="+store.latitude+","+store.longitude+"&hl=us&t=m&dirflg=d", '_blank' );
	}
}
...
//		blink($("#forgot_password_page #password"));
	...
	//user.mobile=$("#forgot_password_page #phoneNumber").val();
	user.code=verCode;
...
//	log("senddata.newPasswd="+senddata.newPasswd+";senddata.verCode="+senddata.verCode+";senddata.mobile="+senddata.mobile);
...
//	console.log("user.sid="+user.sid+";user.email="+user.email+";user.password="+user.password);
...
$(document).delegate('#my_order_detail_page', 'pageshow', function () {
	...
	if(order.status==0||order.status==5){
		...
		//$("#my_order_detail_page #pickUp #pickUpTime").text(nd.toLocaleTimeString().substring(0,5));
...
function doCommandDeleteToMyFavorite(store){
	...
	senddata.sid = user.sid;
	senddata.storeId = store.storeId;
//	log("user.sid="+user.sid+";store.storeId="+store.storeId);
//	if(store.customerId == 0||isNull(store.customerId)){//customerId:0
//		ajax(COMMAND_ADD_COLLECT,ADD_COLLECT_URL,senddata);
//	}else{
...
//	dish.modifiers=[];
	...
	log("dish.modifiers.length============="+dish.modifiers.length);
	for(var i=0;i<dish.modifiers.length;i++){
		...
		navigator.notification.confirm(
				...
				function(button){
					if(button==1){
						...
						resetOrderList();
//						$("#order_page #myDishes li:eq("+index+")").remove();
						if(coupon!=null){
							if(coupon.couponType==0){//具体Item
...
//	log("paymentToken.paidAmt====="+paymentToken.paidAmt);
...
//	log("accessToken===="+user.accessToken.accessToken+";userId===="+user.accessToken.userId);
	$.ajax({
 		type: 'GET',
...
//      		showAlert("Failed to register. Status Code: 422");
       	}
    });
}
...
function doCommandChangeBirthday(){
	...
	text_bak=birthday;
//	var month=$("#change_birthday_page #month").val();
//	var day=$("#change_birthday_page #day").val();
//	var year=$("#change_birthday_page #year").val();
//	if(isNull(month)){
//		showAlert("Please insert the Month");
//		return;
//	}
//	if(isNull(day)){
//		showAlert("Please insert the Day");
//		return;
//	}
//	if(isNull(year)){
//		showAlert("Please insert the Year");
//		return;
//	}
//	
//	if(month<10){
...
//	if (oldPasswd.length < 6 || oldPasswd.length > 20) {
//		showAlert("Please insert a valid password. ");
//		return;
//	}
...
//$(document).delegate('#share_with_facebook_page', 'pageshow', function () {
//	curPage="share_with_facebook_page";
//	$("#share_with_facebook_page #advice").val("");
//});

//var advice="";
function doShareWithFacebook(){
	//advice=$("#share_with_facebook_page #advice").val().trim();
	//if(advice.length>0){
...
//	}else{
//		showAlert("The content can't be empty.");
//	}
}
